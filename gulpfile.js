...
gulp.task('swagger', function (framework, destinationFile, fileFormat, destinationFolder) {
    ...
    plugins.util.log(swaggerCodeGenerator(framework || 'angularjs', destinationFile || 'slapi', fileFormat || '.js', destinationFolder || './swagger/'));
});
...
gulp.task('lint', function () {
    return gulp.src(['api/{,*/}*.js', 'gulpfile.json'])
        .pipe(plugins.jshint())
...
gulp.task('default', function () {
    plugins.util.log('Travis Env ' + process.env.TRAVIS);
    process.env.TRAVIS ? gulp.watch(['serve', 'swagger']) : gulp.watch('serve');
    gulp.watch(['api/{,*/}*.js', 'api/swagger/specs/spec.json', 'api/swagger/swagger-ui/*'], ['lint', 'schema', 'swagger-code-gen', 'serve']);
    gulp.start('schema');
});
